...
enum Direction {Down, Left, Up, Right}; //specifies direction, value of up 0, left 1, down 2, right 3
	...
	int main()
	{
		...
		//move1.rotate();
		town.setSmooth(true);
		town.loadFromFile("thisishom4.png");
		...
		view.reset(sf::FloatRect(0, 0, 1000, 675));
		view.setViewport(sf::FloatRect(0, 0, 1.0f, 1.0f));

		sf::Vector2f position(1000/2, 675/2);
		bool gamerun = false;
		while (window.isOpen())
		{
			...
			if ((sf::Keyboard::isKeyPressed(sf::Keyboard::B)))
			{
				gamerun = true;
					...
					{
						if (move1.getPosition().x + 16 > 1000 / 2)
							position.x = move1.getPosition().x + 16;
						if(move1.getPosition().y + 16 > 675 / 2)
							position.y = move1.getPosition().y + 16;
						else
							position.y = 675 / 2;

						view.setCenter(position);
						/*position.x = move1.getPosition().x + 16 - (1000 / 2);
						position.y = move1.getPosition().x + 16 - (675 / 2);
						if (position.x < 0)
							position.x = 0;
						if (position.y < 0)
							position.y = 0;
						*/
						//view.reset(sf::FloatRect(position.x, position.y, 1000, 675));
						window.setView(view);
						window.clear();
